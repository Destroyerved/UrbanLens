    ...
    <AnimatePresence>
      {open && (
        <motion.aside
          ...
          className="glass-strong pointer-events-auto absolute bottom-3 right-3 top-[64px] z-[40] flex w-[376px] flex-col overflow-hidden rounded-2xl shadow-elev-3"
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-border/80 bg-surface-2/40 px-4 py-3 backdrop-blur-md">
            <div className="flex items-center gap-2.5">
              <div className="grid h-7 w-7 place-items-center rounded-xl bg-accent/20 ring-1 ring-accent/40 shadow-sm">
                <Sparkles size={14} className="text-accent" />
              </div>
              <div>
                <div className="text-[13.5px] font-bold text-foreground">AI Copilot</div>
                <div className="text-[10px] text-muted-foreground">
                  Natural-language control · deterministic spatial analysis
                </div>
              </div>
            </div>
            <button
          ...
          <div ref={listRef} className="panel-scroll flex-1 space-y-3 overflow-y-auto px-4 py-3.5">
                    ...
                    <div className="mt-2.5 flex flex-wrap gap-1.5">
                      {m.actions.map((a) => (
                        <button
                          ...
                          className="glass rounded-xl bg-accent/20 px-2.5 py-1 text-[10.5px] font-bold text-accent ring-1 ring-accent/40 shadow-xs transition-transform hover:scale-[1.02] active:scale-95"
                        >
                          {a.label}
                        </button>
                      ))}
            ...
            {messages.length <= 1 && (
              <div className="space-y-1.5 pt-1">
                <div className="label-caps font-bold">Try asking</div>
                {SUGGESTIONS.map((s) => (
                  <button
                    key={s}
                    onClick={() => submit(s)}
                    className="glass-card block w-full rounded-2xl px-3 py-2.5 text-left text-[11.5px] font-medium text-muted-foreground transition-all hover:scale-[1.01] hover:border-accent/40 hover:text-foreground"
                  >
                    {s}
                  </button>
                ))}
          ...
          <form
            ...
            className="border-t border-border/80 bg-surface-2/40 p-3 backdrop-blur-md"
          >
            <div className="glass flex items-center gap-2 rounded-2xl px-3 focus-within:ring-1 focus-within:ring-accent/60">
              <input
                value={input}
                onChange={(e) => setInput(e.target.value)}
                placeholder="Ask about growth, gaps, sites, parcels…"
                className="h-10 flex-1 bg-transparent text-[12.5px] outline-none placeholder:text-muted-foreground/70"
              />
              <button
                type="submit"
                disabled={busy || !input.trim()}
                aria-label="Send"
                className="grid h-7 w-7 place-items-center rounded-xl bg-accent text-accent-foreground shadow-sm transition-all hover:scale-105 active:scale-90 disabled:opacity-40"
              >
                <CornerDownLeft size={13} />
              </button>
            </div>
            <div className="mt-1.5 text-center text-[9.5px] font-medium text-muted-foreground/75">
              Copilot never invents numbers — answers are computed from the deterministic spatial engine.
            </div>
          </form>
        </motion.aside>
      )}
