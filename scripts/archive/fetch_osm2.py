...
def split_bbox(s, w, n, e, nx=2, ny=2):
    out = []
    lat_step = (n - s) / ny
    lon_step = (e - w) / nx
    for i in range(ny):
...
def fetch_quad(ep, body):
    headers = {"User-Agent": "hackathon-dataset-prep/1.0", "Accept": "application/json"}
    r = requests.post(ep, data={"data": body}, headers=headers, timeout=180)
    r.raise_for_status()
...
def main():
    for city, bbox in CITIES.items():
        ...
        print(f"== {city} ({len(quads)} quads) ==", flush=True)
        for qname, qdata in QUERIES.items():
            all_els = []
            for qi, qb in enumerate(quads):
                body = f"[out:json][timeout:180];{qdata}({qb[0]},{qb[1]},{qb[2]},{qb[3]});out geom;"
                ok = False
                for attempt in range(3):
                    for ep in ENDPOINTS:
                        try:
                            els = fetch_quad(ep, body)
                            all_els.extend(els)
                            print(f"  {qname} quad{qi} ok ({len(els)})", flush=True)
                            ok = True
                            break
                        except Exception:
                            continue
