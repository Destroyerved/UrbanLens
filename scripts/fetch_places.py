...
LAYERS = {
    "localities": 'nwr["place"~"^(neighbourhood|quarter|suburb)$"]',
    "residential": 'nwr["landuse"="residential"]["name"]',
}
...
def main():
    for city, bbox in CITIES.items():
        ...
        path = f"{OUT}/{city}_places.geojson"
        with open(path, "w", encoding="utf-8") as fh:
            json.dump({"type": "FeatureCollection", "features": combined}, fh, ensure_ascii=False)
        print(f"  -> saved {path} ({len(combined)} features)", flush=True)
        time.sleep(6)
